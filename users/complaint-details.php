...
} else
{
            ...
            <label class="col-sm-2 col-sm-2 control-label"><b>Feedback :</b></label>
                          <div class="col-sm-4">
                          <p style="color:red">
                          <?php if($row['status']=="closed")
{?>
<?php $qq = mysqli_query($bd, "select * FROM feedback where cid='" .$row['complaintNumber']. "'");
$arr= mysqli_fetch_array($qq); ?>
<?php if($arr === null){?>
<a href="javascript:void(0);" onClick="popUpWindow('http://localhost/Complaint Management System/users/feedback.php?cid=<?php echo htmlentities($row['complaintNumber']);?>');" title="feedback">
											 <button type="button" class="btn btn-primary">Give Feedback</button></td>
											</a>
<?php }else{
  echo htmlentities ("Already given");
}?>
                      <?php } ?></p>
